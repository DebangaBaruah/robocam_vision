...
def process_video(input_video_path, confidence_threshold=0.5, skip_frames=2,
    ...
    try:
        ...
        vision_system = get_robot_vision_system()
        if vision_system is None:
            return None, f"❌ System initialization failed. Check server logs.\n\nLast error: {_init_error}"
...
try:
    ...
    print(f"[App] UI creation failed: {e}", file=sys.stderr)
    # Fallback minimal interface
    with gr.Blocks(title="🤖 Robot Vision System - Error") as demo:
        gr.Markdown(f"""
        # ❌ System Error
        
        The robot vision system failed to initialize properly.
